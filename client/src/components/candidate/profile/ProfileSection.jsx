...
const ProfileModal = ({ title, modalOpen, closeModal, formData, handleChange, handleSubmit, isFormValid, editingItem, fields }) => {
      ...
      <div className="bg-white rounded-xl w-full max-w-md overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <button type="submit" disabled={!isFormValid} className={`px-4 py-2 rounded-lg text-white ${isFormValid ? "bg-blue-600 hover:bg-blue-700" : "bg-gray-400 cursor-not-allowed"}`}>
...
const ProfileItemList = ({ title, items, fields, openModal, handleDelete, confirmDeleteId }) => {
  ...
  return (
    <div className="p-4" style={{ maxHeight: "200px", overflowY: "auto", scrollbarWidth: "thin" }}>
...
const ProfileSection = ({ title, items, fields, onAdd, onUpdate, onDelete }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingItem) await onUpdate(editingItem._id, { ...formData, _id: editingItem._id });
  ...
  return (
    <div className="profile-section bg-white rounded-2xl p-6 shadow-sm">
      ...
      <ProfileItemList title={title} items={items} fields={fields} openModal={openModal} handleDelete={handleDelete} confirmDeleteId={confirmDeleteId} />
      <ProfileModal title={title} modalOpen={modalOpen} closeModal={closeModal} formData={formData} handleChange={handleChange} handleSubmit={handleSubmit} isFormValid={isFormValid} editingItem={editingItem} fields={fields} />
    </div>
  );
};
